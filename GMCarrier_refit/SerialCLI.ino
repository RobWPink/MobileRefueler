  ...
  if (str.length() > 0) {
    ...
    for(int n = 0; n < numArgs; n++){
      ...
      if(argStr.equalsIgnoreCase("bank1")){
        if(STATE <= IDLE_OFF){activeBank = (activeBank == 1)?0:1;}
      }
      
      else if(argStr.equalsIgnoreCase("bank2")){
        if(STATE <= IDLE_OFF){activeBank = (activeBank == 2)?0:2;}
      }

      else if(argStr.equalsIgnoreCase("bank3")){
        if(STATE <= IDLE_OFF){activeBank = (activeBank == 3)?0:3;}
      }
  
      else if(argStr.equalsIgnoreCase("bank4")){
        if(STATE <= IDLE_OFF){activeBank = (activeBank == 4)?0:4;}
      }
...
void jsonPrint(){
  if(tog[0]){
    ...
    Serial.print(",");
    Serial.print(stallCnt);
    Serial.print(",");
    Serial.print(psiCnt);
    Serial.print(",");
    Serial.print(dmd?"true":"false");
    Serial.print(",");
    ...
    Serial.print(",");
    Serial.print(STALL_CONFIRMATION_COUNT);
    Serial.print(",");
